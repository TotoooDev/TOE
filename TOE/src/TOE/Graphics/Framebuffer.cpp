...
namespace TOE
{
	...
	void Framebuffer::Recreate()
	{
		...
		{
			TOE_ASSERT_NO_MESSAGE(m_ColorAttachments.size() <= 4);
		...
		TOE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
	...
	void Framebuffer::Resize(unsigned int width, unsigned int height)
	{
		...
		{
			spdlog::warn("Attempted to rezize framebuffer to {0}, {1}", width, height);
			return;
		}
	...
	int Framebuffer::ReadPixel(unsigned int attachmentIndex, unsigned int x, unsigned int y)
	{
		TOE_ASSERT_NO_MESSAGE(attachmentIndex < m_ColorAttachments.size());
	...
	unsigned int Framebuffer::GetColorAttachmentID(unsigned int index) const
	{
		TOE_ASSERT_NO_MESSAGE(index < m_ColorAttachments.size());
